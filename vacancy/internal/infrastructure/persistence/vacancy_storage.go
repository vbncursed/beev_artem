...
func (s *VacancyStorage) Close() {
	if s.db != nil {
		s.db.Close()
	}
}

// NewVacancyStorage opens a pgx pool, verifies connectivity with a Ping, and
...
func NewVacancyStorage(connString string) (*VacancyStorage, error) {
...
func applyMigrations(ctx context.Context, connString string) error {
	...
	goose.SetTableName("vacancy_goose_db_version")
