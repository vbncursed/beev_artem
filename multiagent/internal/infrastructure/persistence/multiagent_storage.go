...
func (s *MultiAgentStorage) Close() {
	if s.db != nil {
		s.db.Close()
	}
}

// NewMultiAgentStorage opens a pgx pool, verifies connectivity with a Ping,
// and applies any outstanding migrations through goose.
func NewMultiAgentStorage(connString string) (*MultiAgentStorage, error) {
	...
	return &MultiAgentStorage{db: db}, nil
}
...
func applyMigrations(ctx context.Context, connString string) error {
	...
	goose.SetTableName("multiagent_goose_db_version")
