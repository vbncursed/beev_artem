...
func (a *ResumeServiceAPI) CreateCandidateFromResume(ctx context.Context, req *pb_models.CreateCandidateFromResumeRequest) (*pb_models.CandidateResumeResponse, error) {
	...
	// Boundary check: reject obviously bad payloads before paying for
	// extractor work. Service still re-checks (defense in depth).
	if size := len(req.GetFileData()); size == 0 || size > usecase.MaxResumeSizeBytes {
		return nil, newError(codes.InvalidArgument, ErrCodeInvalidInput, "Resume file size out of range.")
	}
