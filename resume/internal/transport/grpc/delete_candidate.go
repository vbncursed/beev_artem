...
func (a *ResumeServiceAPI) DeleteCandidate(ctx context.Context, req *pb_models.DeleteCandidateRequest) (*pb_models.DeleteCandidateResponse, error) {
	userCtx, ok := middleware.Get(ctx)
	if !ok {
		return nil, newError(codes.Unauthenticated, ErrCodeUnauthorized, "Authentication required.")
	}

	err := a.resumeService.DeleteCandidate(ctx, domain.DeleteCandidateInput{
	...
	return &pb_models.DeleteCandidateResponse{}, nil
}
