...
func (r *VacancyRepository) Create(ctx context.Context, v vacancy.Vacancy) error {
	...
	_, err = tx.Exec(ctx, `
INSERT INTO vacancies (id, title, description, created_at)
VALUES ($1, $2, $3, $4)
`, v.ID, strings.TrimSpace(v.Title), v.Description, v.CreatedAt)
	if err != nil {
		return err
	}
...
func (r *VacancyRepository) UpdateSkills(ctx context.Context, id uuid.UUID, skills []vacancy.SkillWeight) error {
	tx, err := r.pool.Begin(ctx)
	if err != nil {
		return err
	}
...
func (r *VacancyRepository) GetByID(ctx context.Context, id uuid.UUID) (vacancy.Vacancy, error) {
	row := r.pool.QueryRow(ctx, `
SELECT id, title, description, created_at FROM vacancies WHERE id = $1
`, id)
	var v vacancy.Vacancy
	var created time.Time
	if err := row.Scan(&v.ID, &v.Title, &v.Description, &created); err != nil {
...
func (r *VacancyRepository) List(ctx context.Context, limit, offset int) ([]vacancy.Vacancy, error) {
	if limit <= 0 {
		limit = 50
	}
	rows, err := r.pool.Query(ctx, `
...
`, limit, offset)
	...
	for rows.Next() {
		...
		var created time.Time
		var skillsJSON []byte
		if err := rows.Scan(&v.ID, &v.Title, &v.Description, &created, &skillsJSON); err != nil {
			return nil, err
		}
		v.CreatedAt = created.UTC()
