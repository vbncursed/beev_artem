...
func (s *LogoutAllSuite) TestSessionBelongsToOtherUser() {
	...
	s.sessionStorage.GetSessionByRefreshHashMock.Expect(ctx, hashRefreshToken(tok)).Return(&domain.Session{UserID: 999}, nil)
...
func (s *LogoutAllSuite) TestRevokeAllStorageError() {
	...
	s.sessionStorage.GetSessionByRefreshHashMock.Expect(ctx, hashRefreshToken(tok)).Return(&domain.Session{UserID: 7}, nil)
