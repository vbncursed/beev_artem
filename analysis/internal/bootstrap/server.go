...
func AppRun(api *transport_grpc.AnalysisServiceAPI, authClient auth_api.AuthServiceClient, cfg *config.Config, onShutdown ...func()) error {
	...
	// by the time we register the server, pgxpool, the auth client and the
	// multiagent client have already been constructed by bootstrap.Init*.
