...
| `admin/` | Operational dashboard service: aggregate stats (`SELECT count(*)` across the shared `hr` DB), `ListUsers` with activity counters, `Promote`/`Demote` proxied to `auth.UpdateUserRole`. Admin-only at the auth interceptor. | `:50056` | — |
| `frontend/` | Vite-built SPA (Cadence brand) on gateway:8080. Clean-architecture mirror in TypeScript: domain → application → infrastructure → presentation. ru/en i18n, light/dark theme, drag&drop resume upload, AI analysis details, admin dashboard at `/admin`. | — | — |
...
- **2026-05-06:** Multiagent `GenerateDecision` contract gained two LLM-extracted profile fields — `years_experience` (float) and `candidate_summary` (1–2 sentence blurb) — wired through proto/domain/parser/conversions in multiagent + analysis + vacancy proto copies. `parseDecision` clamps YOE to `[0, 60]` to defend against hallucinated values. After a successful LLM call, `analysis.StartAnalysis` overwrites `profile_json.{years_experience, summary}` via per-method `jsonb_set` storage helpers (`UpdateProfileYearsExperience`, `UpdateProfileSummary`); empty/zero values keep the heuristic fallback. The heuristic regex `(\d{1,2})\s*...\s*(?:год|лет|year)` was matching "(20 лет)" from `Дата рождения: ... (20 лет)` — i.e. age, not experience — so the LLM override is now the authoritative source. `summarize()` in `scorer/profile.go` now truncates by **runes** instead of bytes (320-byte slice was landing mid-Cyrillic codepoint and producing invalid UTF-8 in the heuristic preview).
- **2026-05-06:** Analysis→multiagent timeout bumped from `5s` to `45s` (Yandex full hire/maybe/no decision takes 5–15s; the old cap killed every call as `context canceled`, leaving every analysis on the heuristic fallback). `analysis.StartAnalysis` now forwards the **full resume text** (not the 320-rune `Profile.Summary` preview) to multiagent so the model can see date ranges (`Февраль 2025 — Октябрь 2025`, `11 МЕСЯЦЕВ`) needed to compute years_experience. All resume-derived strings (`ResumeText`, `CandidateSummary`, `ScoreExplanation`, skills slices) are passed through `strings.ToValidUTF8` before gRPC marshaling — `pdftotext` occasionally emits stray bytes from glyphs without proper encoding, and a single bad byte was failing protobuf marshaling for one resume per batch with `string field contains invalid UTF-8`. Added `slog.WarnContext`/`InfoContext` to multiagent's `GenerateDecision` and analysis's `StartAnalysis`/`refreshAIDecisionAsync` so upstream LLM failures (Yandex 4xx, timeouts, marshal errors) are visible in container logs instead of being silently swallowed.
- **2026-05-06:** Frontend batch resume upload per vacancy. Hits `POST /api/v1/vacancies/{id}/resumes/batch` once for N files, then fans out `analysisGateway.start` via `Promise.all` for each successfully ingested resume. `ResumeUploader` accepts `multiple`; per-file size validation surfaces the failing filename. New i18n keys `details.batchUploadSuccess` / `details.batchUploadItemError`; existing `upload.tooLarge` now takes `{name}`. Gateway port `ingestResumeBatch` returns `BatchIngestResult` with per-item `error` for partial failures; clean architecture preserved (domain → application port → infrastructure HTTP gateway → presentation).
- **2026-05-06:** Vacancy role detection moved from a keyword table to the LLM. New `multiagent.ClassifyRole` RPC (temperature 0, JSON contract, validated against `PromptStore.ListRoles() ∪ {"default"}`); vacancy usecase gets a `RoleClassifier` port wired to a gRPC adapter at `vacancy/internal/infrastructure/multiagent_client/`. Calls run with a 5s timeout; any failure (Unavailable, DeadlineExceeded, parse error, unknown role) silently falls back to the deterministic `DetectRole` keyword detector — vacancy CRUD never blocks on a flaky LLM. Adding a new role still = drop `multiagent/.../templates/<role>.txt`; the classifier prompt enumerates `ListRoles()` at request time.
...
- **2026-05-03:** New `admin/` service shipped — sixth Go module. Mirror clean architecture, gRPC `:50056`, every RPC admin-only via the auth interceptor (`PermissionDenied` for non-admin tokens). Reads aggregate counts directly from the shared `hr` DB across all per-service tables (documented exception to per-service ownership in `admin/README.md`); proxies role changes to `auth.UpdateUserRole`. Frontend gets `/admin` page with stat cards + user table + Promote/Demote buttons; `<AdminOnly>` route guard + admin-link in TopNav visible only when `user.role === 'admin'`. Bootstrap CLI `auth/cmd/admin` (in-image binary, `make admin-promote EMAIL=...`) handles the chicken-and-egg of creating the first admin.
- **2026-05-03:** Vacancy length validation switched from `len()` (bytes) to `utf8.RuneCountInString` (runes) so a 2090-character Cyrillic description (4181 UTF-8 bytes) no longer hits the 4000-byte rejection that the frontend counter said was within limits. Limits now named constants (`maxTitleRunes`, `maxDescriptionRunes`).
- **2026-05-03:** PDF extraction switched to `pdftotext` (poppler-utils, installed in resume Dockerfile) — `ledongthuc/pdf` returns `X=0/W=0/FontSize=0` for every glyph in PDFs without positional metadata, making heuristic word-boundary recovery impossible. `extractViaLedongthuc` kept as a best-effort fallback for dev environments without the binary.
- **2026-05-03:** Gateway blank-imports `google.golang.org/genproto/googleapis/rpc/errdetails` so `google.protobuf.Any` payloads (every `errdetails.ErrorInfo` from backend) marshal correctly to JSON. Without it, structured error responses became `"unable to resolve type.googleapis.com/google.rpc.ErrorInfo: not found"`. Frontend `parseAnalysisStatus` / `parseVacancyStatus` accept both string ("ANALYSIS_STATUS_DONE") and number (3) shapes — grpc-gateway emits proto enums as string names by default.
